...
- successful responses saved in jsonl files
    ...
    and one file per day under OUTPUT_DIR/
      e.g. OUTPUT_DIR/2024-01-01.jsonl)
...
API_URL = CONFIG["requests"]["api_url"]
MAX_RETRIES = CONFIG["requests"]["max_retries"]
REQUEST_BACKOFF_BASE = CONFIG["requests"]["backoff_base_seconds"]
REQUEST_TIMEOUT = CONFIG["requests"]["timeout_seconds"]
OUTPUT_DIR = Path(CONFIG["paths"]["raw_dir"])
START_DATETIME = CONFIG["collection"]["aligned_start_datetime"]
END_DATETIME = CONFIG["collection"]["aligned_end_datetime"]
DELTA_HOURS = CONFIG["collection"]["delta_hours"]
...
def main():
    ...
    for start_datetime, end_datetime in intervals:
        ...
        valid_data = validate_api_data(data) if data is not None else None

        if valid_data is not None:
            save_response(start_datetime, end_datetime, valid_data)
            success_count += 1
        else:
            fail_count += 1
...
def validate_api_data(data: list[dict]) -> list[dict] | None:
    """
    validates each item in data received from api request
    returns valid_items | None
    """
    MIN_AQI = 0
    MAX_AQI = 500
    ...
    for item in data:
        try:
            assert "source" in item, "missing source id"
            assert -90 <= item["latitude"] <= 90, f"bad lat: {item['latitude']}"
            assert -180 <= item["longitude"] <= 180, f"bad lon: {item['longitude']}"
            assert item["quality"]["index"] is not None
            assert MIN_AQI <= item["quality"]["index"] <= MAX_AQI, (
                f"AQI out of range [0, {MAX_AQI}): got {item['quality']['index']}"
            )
            valid_items.append(item)
        except (AssertionError, KeyError, TypeError) as e:
            log.warning("bad item in %s:\n  %s", data, e)

    if not valid_items:
        log.warning("returning None for %s after validation", data)
        return None
...
def save_response(start: datetime, end: datetime, data: list):
    """
    append response data in jsonl file

    files partitioned by 'start' day
      (e.g. start='2024-01-01T23:00:00Z' records
      appended to OUTPUT_DIR/2024-01-01.jsonl)
    """

    day_str = start.strftime("%Y-%m-%d")
    out_path = OUTPUT_DIR / f"{day_str}.jsonl"
